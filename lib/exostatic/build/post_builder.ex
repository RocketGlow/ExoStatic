defmodule Exostatic.Build.PostBuilder do
  ...
  def run(src, dest, mode) do
    srcdir = "#{src}posts/"
    dstdir = "#{dest}posts/"
    Agent.update(Exostatic.PostInfoStorage, fn _ -> [] end)
  ...
  def post_task(srcdir, dstdir, file) do
    ...
    Agent.update Exostatic.PostInfoStorage, &([info|&1])
  ...
  defp render_post(contents, info) do
    template = Exostatic.get_data "template_post"
    template
